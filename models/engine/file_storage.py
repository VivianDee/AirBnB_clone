...
class FileStorage:
    ...
    def new(self, obj):
        """Sets the obj and the ke in __objects"""
        if obj:
            self.__objects[(obj.__class__.__name__ + "." + obj.id)] = obj.__dict__

    def save(self):
        """Serializes __objects to the JSON file"""
        copy = {outer_key: dict(inner_dict) for outer_key, inner_dict in self.__objects.items()}
        for key, value in copy.items():
            if 'updated_at' in value and type(value['updated_at']) != str:
                value['updated_at'] = value['updated_at'].strftime("%Y-%m-%dT%H:%M:%S.%f")
            if 'created_at' in value and type(value['created_at']) != str:
                value['created_at'] = value['created_at'].strftime("%Y-%m-%dT%H:%M:%S.%f")
